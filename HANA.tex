...
\begin{document}
    ...
    \section{6th Jan 2023}
    \subsection{Harmonic analysis on finite abelian group}

    Let $ G  $ be  a finite abelian group. 

    Consider $ \pi : G \to U(V_{\pi}) $ and $ \pi: \mathcal{F}(G) \to end(V_{\pi})  $.  which i

    Fix an eigenspace, $ m: \pi( \mathcal{F}(G)) \to \C $ so $ \exists $  $ v \neq 0 $ for $ \pi(f)v = \lambda v $
\end{document}