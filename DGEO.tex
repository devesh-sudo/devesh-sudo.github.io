...
\begin{document}
    ...
    It is more standard to the curvature operator as the map $ Rm : \wedge^{2}T^{*}M^{n} \to \wedge^{2}T^{*}M^{n}$ after lowering/raising appropriate indices. In the case of surfaces, the wedge product is one-dimensional, so the map is determined by a scalar which will be equal to the Gaussian curvature/sectional curvature. 
