...
\begin{proof}
    ...
    Then $ M $ is a Lie subgroup of $ G \times H $ and $ M \xrightarrow{p} G $ obtained by restriction of $ \pi_{1} $ is a group homomorphism and also a local diffeomorphism. So $ p: M \to G $ is a covering projection but $ G $ is simply connected so $ p $ is a differmorphism 
...
\begin{thm}
    ...
    Also 
    \[ \begin{tikzcd}
        {\R} & & { \mathfrak{h}} && { \mathfrak{g}} \\
        \\
        H && G
        \arrow["\exp", from=1-1, to=1-3]
    \end{tikzcd}\]
