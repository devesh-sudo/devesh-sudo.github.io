...
\end{tikzcd}\]
    ...
    \end{thm}
    \begin{proof}
        Consider the distribution $ \mathcal{D} $ defined as 
        \[ \mathcal{D}_{g} = \{X_{g}| X \in \mathfrak{h} \} \]
        on $ G $. Suppose $ X_{1}, \ldots, X_{c} $ is a basis of $ \mathfrak{h} $. Then $ \mathcal{D} $ is generated by $ X_{1}, \ldots,X_{c} $ and $ \mathcal{D}  $ is involutive.
    \end{proof}
    \begin{corollary}
